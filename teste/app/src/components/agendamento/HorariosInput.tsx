...
export default function HorariosInput(props: HorariosInputProps) {
    ...
    function renderizarHorario(horario: string) {
        ...
        return (
            <div
                key={horario}
                className={cn(
                    'flex justify-center items-center cursor-pointer h-8 border border-purple-800 rounded select-none',
                    {
                        'bg-purple-800': destacarHora,
                        'bg-red-500': naoSelecionavel || periodoBloqueado,
                        'text-white bg-green-600': selecionado,
            ...
            >
                <span
                    className={cn('text-sm text-pink-200', {
    ...
    return (
        <div className="flex flex-col gap-5">
            <span className="text-sm uppercase text-pink-200">Horários Disponíveis</span>
            <div className="flex flex-col gap-3 select-none">
                <span className="text-xs uppercase text-pink-200">Manhã</span>
                <div className="grid grid-cols-8 gap-1">{manha.map(renderizarHorario)}</div>

                <span className="text-xs uppercase text-pink-200">Tarde</span>
                <div className="grid grid-cols-8 gap-1">{tarde.map(renderizarHorario)}</div>

                <span className="text-xs uppercase text-pink-200">Noite</span>
